...
class BuildToolBase(object):
    ...
    @classmethod
    def copy_build_logs(cls, **kwargs):
        """ Copy build logs generated by BuildTool
...
class MockBuildTool(BuildToolBase):
    ...
    @classmethod
    def copy_build_logs(cls, **env):
        """ Copy logs from the build process.
        Returns:
        dict with:
        'build_logs' -> list with logs
        """
        log_files = []
        for log in PathHelper.find_all_files(env[cls.TEMPDIR_RESULTDIR], '*.log'):
            log_name = os.path.join(env['results_dir'],
                                    env['source_type'],
                                    os.path.basename(log))
            shutil.copyfile(log, log_name)
    ...
    def build(cls, **kwargs):
        ...
        'srpm' -> path to SRPM
        'rpm' -> list with paths to RPMs
        'logs' -> list with paths to build_logs
        """
        ...
        # copy logs from RPM building
        env['results_dir'] = kwargs.get('results_dir', '')
        os.makedirs(os.path.join(env['results_dir'], env['source_type']))
        build_logs = cls.copy_build_logs(**env)
...
class Builder(object):
    ...
    def build_packages(self, **kwargs):
        ...
        {new: {'srpm': <path_to_srpm>,
                ...
                'logs': <path_to_build_log>, <path_to_build_log>},
        ...
        for path in ['old', 'new']:
            ...
            input_structure['results_dir'] = kwargs.get('results_dir')
            input_structure['source_type'] = path
